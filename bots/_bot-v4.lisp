...
(let negamax-inner (lambda (board lower upper depths)
    (if (eq depths zero) (print (material-heuristic board))
        (negamax-inner-helper board (get-moves board) lower upper depths)))

(let negamax-outer-helper (lambda (board moves upper depths best)
    (if (nilq moves) best 
        (let move (car moves)
            (let score 
            
            
            (sub zero (negamax-inner 
                (make-move board move)
                (sub zero upper)
                -1000
                (sub depths one)))
...
(let negamax-outer (lambda (board depths)
    (negamax-outer-helper board (get-moves board) -9223372036854775806 depths ()))


(let _start (lambda (board timer)
    (negamax-outer board 4))
